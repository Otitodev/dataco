...
- **The scan agent** (`POST /scan` → `services/scan.py`): reads live DataHub, runs `detection.check_asset` vs the stored `MonitoringState`, and on a hit creates the issue, explains it, **writes a tag + assertion back to DataHub**, then advances the baseline so a re-scan doesn't re-alert. Detection is unwired to any auto-schedule — it only runs when `/scan` is called. `seed_live.py` primes deterministic baselines on the `app/monitored_assets.py` watchlist (real `b2fd91.*` URNs); it self-loads `.env`. Dedupe is self-healing: if an active issue for the asset+type already exists, the scan skips creating a duplicate but still completes the write-back if that issue was never written back (`written_back_at is None`). The assertion URN is deterministic (`_assertion_id`, sha1 of asset+issue_type) so re-writes upsert one assertion in place instead of piling up duplicates.
