...
- **`app/services/`** — the shared, framework-free core called by both the API and the MCP server. `reasoning.py` (grounded `explain_issue`/`brief_issue`), `scan.py` (the agent loop + `resolve_urns` watchlist precedence), `writeback.py` (tag + assertion orchestration), `scheduler.py` (opt-in in-process loop that runs `scan_all` every `SCAN_INTERVAL_SECONDS`, wired via `main.py` lifespan; default off). This is where behaviour lives.
